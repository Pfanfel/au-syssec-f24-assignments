...
def encript_plaintext_without_key(plaintext_to_encript, iv, ct, base_url):
    msg = iv + ct

    plaintext_to_encript_blocks = []
    for i in range(0, len(plaintext_to_encript), BLOCK_SIZE):
        plaintext_to_encript_blocks.append(plaintext_to_encript[i : i + BLOCK_SIZE])
    ...
    iv = blocks[0]
    intermediate = plaintext_to_encript_blocks[0]
    for ct, plaintext_block in zip(blocks[1:], plaintext_to_encript_blocks[1:]):
        print(f"ct: {ct}")
        print(f"plaintext_block: {plaintext_block}")
        zeroing_iv_block = single_block_attack_example(ct, base_url)

        # pt = bytes(iv_byte ^ zeroing_iv_block_byte for iv_byte, zeroing_iv_block_byte in zip(iv, zeroing_iv_block))
        temp_xor_block = xor_block(intermediate, zeroing_iv_block)
        result += temp_xor_block
        print(
            f"{temp_xor_block.decode('utf-8')} / {temp_xor_block} = {intermediate} ^ {zeroing_iv_block} -> result: {result}, "
        )
        # print(f"result: {result")
        iv = ct
        intermediate = plaintext_block
...
def CBC_R_encryption(plaintext, base_url):
    ...
    for i in range(N - 1, 0, -1):
        ...
        Ci_1 = bytes([_a ^ _b for _a, _b in zip(blocks[i], decrypted_block)])
    ...
    IV = bytes([_a ^ _b for _a, _b in zip(blocks[0], decrypted_block)])
